...
export const Sidebar = ({ activeSection, onSectionChange }: SidebarProps) => {
  ...
  return (
    <div className={`${isCollapsed ? 'w-16' : 'w-64'} max-h-screen h-[calc(100vh-1rem)] bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700 transition-all duration-300 flex flex-col`}>
      {/* Header */}
            ...
            {menuItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center px-1 py-2 text-sm font-medium rounded-lg transition-colors duration-200 cursor-pointer ${
                    isActive
                      ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 border-2 border-blue-300'
          ...
          <nav className="space-y-1">
            {toolItems.map((item) => {
              ...
              return (
                <button
                  key={item.id}
                  className={`w-full flex items-center px-3 py-2 text-sm font-medium rounded-lg transition-colors duration-200 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer`}
                >
                  <Icon className={`${isCollapsed ? 'w-5 h-5' : 'w-5 h-5 mr-3'} flex-shrink-0`} />
                  {!isCollapsed && <span>{item.label}</span>}
                </button>
